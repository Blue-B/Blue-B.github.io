...
sequenceDiagram
    ...
    CustomerA->>Staff: Order & Payment
    activate Staff
    Staff-->>Staff: Making Drink (Blocking)
    Staff-->>CustomerA: Serve Drink
    deactivate Staff
    
    note right of Staff: CustomerB must wait until CustomerA is done
    
    CustomerB->>Staff: Order & Payment
```
...
sequenceDiagram
    ...
    CustomerA->>Cashier: Order & Payment
    activate Cashier
    Cashier-->>CustomerA: Give Buzzer
    deactivate Cashier

    CustomerB->>Cashier: Order & Payment
    activate Cashier
    Cashier-->>CustomerB: Give Buzzer
    deactivate Cashier

    note right of Cashier: Focus only on taking orders

    par
        Barista-->>Barista: Make CustomerA drink
    and
        Barista-->>Barista: Make CustomerB drink
    end

    note left of Barista: Multiple drinks processed simultaneously

    Barista-->>CustomerA: Buzzer rings - Drink ready
    Barista-->>CustomerB: Buzzer rings - Drink ready
```
